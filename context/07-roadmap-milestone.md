...
- [x] Implementasi `features.py` untuk fitur numerik. `price_deviation` dihitung terhadap median harga **per kelompok varian produk** (bukan median gabungan semua toko) — lihat `product_matching.py` di bawah; `rating`, `review_count`, `is_official_store`, `response_rate`, `total_sold` pass-through dari data mentah. Diverifikasi lewat `/search` dengan fixture data.
- [x] Implementasi `product_matching.py` (perluasan tak terduga dari `features.py` — muncul dari diskusi soal validitas perbandingan harga): dua tahap embedding-based sebelum hitung median — (1) `filter_relevant_products` buang produk yang secara semantik bukan hasil pencarian (misal MacBook nyasar pas cari iPhone); (2) `group_products_by_variant` mengelompokkan produk sebanding via cosine similarity + union-find, dengan veto regex/kosakata tertutup untuk konflik spek (`128GB` vs `256GB`) dan kondisi (`baru` vs `second`) yang tidak boleh digabung meski embedding-nya mirip. `app/embedding_model.py` ditambah sebagai loader model bersama (dipakai `review_analysis` & `product_matching`, tidak di-load dobel).
  - Sempat dicoba pakai NER (`cahya/bert-base-indonesian-NER`) sebagai alternatif regex — terbukti empiris tidak cocok (model ini menandai "iPhone 13 128GB Second" sebagai satu entity PRODUCT utuh, tidak memisahkan `128GB` sebagai token tersendiri; di teks non-HP malah salah kategori total). Regex dipilih karena deterministik & category-agnostic.
  - Threshold `VARIANT_SIMILARITY_THRESHOLD` sempat 0.85 (menyebabkan toko dengan harga sangat mencurigakan di fixture kehilangan sinyal `price_deviation` karena jadi grup sendirian), diturunkan ke 0.70 setelah veto regex aktif sebagai pengaman.
...
- [x] Unit test untuk tiap rule scoring. `tests/test_scoring.py` (28 test — semua threshold per sinyal + kedua compound rule + clamp skor di 0 dan 85) dan `tests/test_labeling.py` (11 test — batas threshold label, filter/urutan reasons, fallback reason). Semua 39 lolos, murni logika jadi jalan <1 detik tanpa load model.

### Fase 4 — Interface & Integrasi (2-3 hari)
- [x] Endpoint FastAPI (`/search?query=...`) menjalankan pipeline lengkap. Sudah sejak Fase 3 (STEP 1-4 penuh via `app/main.py`).
- [x] UI Streamlit untuk input query & tampilkan hasil. `frontend/streamlit_app.py` dari placeholder jadi fungsional: input query → panggil `/search` → tampilkan tiap toko sebagai card dengan label berwarna (hijau/kuning/merah), skor, harga, daftar alasan, link toko. Diverifikasi jalan end-to-end lewat browser sungguhan (uvicorn + streamlit dijalankan, diuji via Chrome) — hasil render cocok dengan data yang sudah divalidasi lewat API.
  - Bug lingkungan ketemu saat testing: default `API_URL` (`http://localhost:8000`) bisa nyasar ke proses lain yang kebetulan listen di port 8000 lewat IPv6 di mesin dev ini. Diperbaiki ke `http://127.0.0.1:8000` sebagai default yang tidak ambigu; tidak berpengaruh ke Docker (`API_URL` di-override eksplisit ke `http://api:8000` oleh `docker-compose.yml`).
